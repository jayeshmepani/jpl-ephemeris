...
jme_moshier_gplan(double J, struct plantbl *plan, double pobj[])
{
  ...
  T = (J - jme_moshier_J2000) / TIMESCALE;
  /* Calculate sin( i*MM ), etc. for needed multiple angles.  */
  for (i = 0; i < 9; i++)
    {
      if ((j = plan->max_harmonic[i]) > 0)
	{
	  sr = (mods3600 (freqs[i] * T) + phases[i]) * jme_moshier_STR;
	  sscc (i, sr, j);
	}
    }
  ...
  pobj[0] = jme_moshier_STR * sl;
  pobj[1] = jme_moshier_STR * sb;
  pobj[2] = jme_moshier_STR * plan->distance * sr + plan->distance;
  return (0);
}
