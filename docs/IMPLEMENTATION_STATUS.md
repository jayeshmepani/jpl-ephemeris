...
- `jme_get_ayanamsa_ex` no longer silently maps unsupported sidereal model IDs to Lahiri. Lahiri, Fagan-Bradley, user-defined mode, source-definable epoch-zero modes (`J2000`, `J1900`, `B1950`), fixed-star anchor modes (`Aldebaran 15 Tau`, `True/SS Citra`, `True/SS Revati`, `True Mula`, `True Pushya`), `Galactic Center 0 Sagittarius`, Krishnamurti/Newcomb, Raman, Yukteshwar, UT wrapper behavior, null-output rejection, and unsupported-model rejection are covered. The numeric ayanamsa rows remain open until every remaining declared traditional mode has independent source-backed formula/epoch data and known-value validation.
...
- Broader independent known-value validation is still incomplete across full numeric ayanamsa model breadth and dense analytical fallback edge cases.
